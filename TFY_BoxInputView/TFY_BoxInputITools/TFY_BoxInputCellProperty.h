...
typedef NS_ENUM(NSInteger, TFY_BoxSecurityType) {
    TFY_BoxSecuritySymbolType,//默认
    TFY_BoxSecurityCustomViewType,//自定义模式
};

typedef UIView *_Nonnull(^CustomSecurityViewBlock)(void);
typedef TFY_LineView *_Nonnull(^CustomLineViewBlock)(void);
typedef void(^ConfigCellShadowBlock)(CALayer *layer);

@interface TFY_BoxInputCellProperty : NSObject<NSCopying>
#pragma mark - UI
/**
 cell边框宽度
 默认：0.5
...
/**
 保存当前显示的字符
 若想一次性修改所有输入值，请使用 TFY_BoxInputView中的'reloadInputString'方法
 禁止修改该值！！！（除非你知道该怎么使用它。）
 */
...
/**
 密文类型
 默认：TFY_BoxSecuritySymbolType
 类型说明：
 TFY_BoxSecuritySymbolType 符号类型，根据securitySymbol，originValue的内容来显示
 TFY_BoxSecurityCustomViewType 自定义View类型，可以自定义密文状态下的图片，View
 */
@property (assign, nonatomic) TFY_BoxSecurityType securityType;
